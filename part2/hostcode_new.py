...
# Load the overlay
overlay = Overlay("/path/to/fc6/overlay.bit")  # Replace with the actual path

# Access the matrix multiplication IP instance in the overlay
fc6_inst = overlay.pl_fc6_0  
...
def fc6_accelerator(input, weights, bias, output):
    ...
    fc6_inst.mmio.write_reg(0x10, input.physical_address)
    fc6_inst.mmio.write_reg(0x14, 0)
    fc6_inst.mmio.write_reg(0x1C, weights.physical_address)
    fc6_inst.mmio.write_reg(0x20, 0)
    fc6_inst.mmio.write_reg(0x24, bias.physical_address)  
    fc6_inst.mmio.write_reg(0x28, output.physical_address)
    fc6_inst.mmio.write_reg(0x2C, 0)

    # Trigger the accelerator
    fc6_inst.write(0x00, 1)

    # Wait for the accelerator to finish
    while fc6_inst.read(0x00) == 14:
        pass

    # Synchronize data back from the FPGA
    input.sync_from_device()
    weights.sync_from_device()
    bias.sync_from_device()
    output.sync_from_device()


    print(f"Execution Time (FPGA Accelerator): {execution_time_accelerator:.6f} seconds")
