...
def fc6_accelerator(input, weights, bias, output):
    input_buf = allocate(shape=input.shape, dtype=np.float32)
    output_buf = allocate(shape=output.shape, dtype=np.float32)
    ...
    fc6_inst.write(0x1C, weights.physical_address)
...
# Function to parse network parameters
